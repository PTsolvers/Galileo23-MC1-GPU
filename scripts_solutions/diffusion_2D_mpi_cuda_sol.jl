# mpirun -np 4 -mca btl_openib_warn_default_gid_prefix 0 julia --project diffusion_2D_mpi_cuda.jl
using Plots, Printf
...
@views function main(; dims=(1, 1))
    ...
    D        = d0 .* CUDA.ones(Float64, ny - 1, nz - 1)
    qy       = CUDA.zeros(Float64, ny - 1, nz - 2)
    qz       = CUDA.zeros(Float64, ny - 2, nz - 1)
    # MPI buffers
    send_bufs = [[CUDA.zeros(Float64, nz) for side in 1:2], [CUDA.zeros(Float64, ny) for side in 1:2]]
    recv_bufs = deepcopy(send_bufs)
    # action
    iters_evo = Float64[]; errs_evo = Float64[]; iter = 1
    while iter <= maxiter
        CUDA.@sync @cuda threads=nthreads blocks=nblocks update_q!(qy, qz, C, D, dy, dz)
        CUDA.@sync @cuda threads=nthreads blocks=nblocks update_C!(C, qy, qz, D, dτ, dy, dz)
    ...
    MPI.Barrier(comm)
    gather!(C_g, Array(C[2:end-1, 2:end-1]), comm)
